 ...
 * контроль температуры потока воздуха от нагревателя
...
/**
* Начальное значение температуры, которую системы будет поддерживать при старте системы
* Возможно лучше будет использовать энергонезависимую память, чтобы автоматически вспоминать
* ранее настроенную температуру.
* На текущий момент это указывается константой.
* Т.к. не реализована кнопка старт\стоп, что нагрев автоматически не включался
* при включении системы, указываю температуру ниже средней комнатной.
*/
#define TARGET_TEMP_INIT_VALUE 20
...
void setup()
{
  ...
  //Указываем температуру, которую будет поддреживать система при запуске железяк
  endcoderSetValue( TARGET_TEMP_INIT_VALUE ); //для энкодера, чтобы человек начинал настраивать ту же Т, что и сама система
  setTargetTemp(TARGET_TEMP_INIT_VALUE ); //указывает Т для систмы
}
